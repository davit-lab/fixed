...
const RouteList = () => {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel('route-posts')
      .on('postgres_changes', 
        { event: '*', schema: 'public', table: 'posts', filter: `type=eq.route` },
        () => {
          fetchPosts();
        }
      )
        ...
        <div className="max-w-screen-xl mx-auto px-4 md:px-6">
          <div className="flex flex-col lg:flex-row items-start lg:items-end justify-between gap-8 mb-10">
            ...
            <div className="space-y-4">
              ...
              <h1 className="text-4xl md:text-5xl lg:text-6xl font-black tracking-tight">
                Active{' '}
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-500 to-teal-500">Routes</span>
              </h1>
              <p className="text-muted-foreground text-base md:text-lg font-medium max-w-md leading-relaxed">
                Find available vehicles for your cargo across the entire region.
              </p>
              <Button 
                asChild 
                className="h-12 px-6 rounded-xl font-bold bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-white shadow-lg shadow-emerald-500/20 border-0 transition-all hover:scale-[1.02] active:scale-[0.98]"
              >
                <Link to="/create-post">
                  Post Route
            ...
            <div className="flex items-center gap-3 w-full lg:w-auto">
              <div className="relative flex-1 lg:w-80 group">
                ...
                <input 
                  placeholder="Search..." 
                  className="w-full pl-11 pr-4 h-12 rounded-xl border border-border/50 bg-card/80 backdrop-blur-sm shadow-sm focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500/50 transition-all text-sm font-medium outline-none"
                  value={searchTerm}
          ...
          <div className="flex flex-wrap items-center gap-4 p-4 rounded-2xl bg-card/50 border border-border/30 backdrop-blur-sm mb-10">
            <div className="flex items-center gap-2 text-sm">
              ...
              <span className="font-bold">{filteredPosts.length} routes</span>
            </div>
          </div>
        </div>
      </section>
            ...
            {filteredPosts.length === 0 ? (
              ...
              >
                ...
                <div className="space-y-2">
                  <h3 className="text-2xl font-bold">No routes found</h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredPosts.map((post, index) => (
                  <motion.div
                    ...
                    layout
                  >
                    <Card className="group h-full rounded-2xl border border-border/50 bg-card hover:shadow-xl hover:shadow-emerald-500/5 hover:border-emerald-500/20 transition-all duration-500 overflow-hidden flex flex-col">
                      <div className="p-6 pb-4 space-y-6 flex-1">
                        {/* Header */}
                        <div className="flex items-center justify-between">
                          <div className="px-3 py-1.5 rounded-lg bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 text-xs font-bold">
                            Route
                          </div>
                        ...
                        <div className="space-y-4 relative">
                          <div className="absolute left-[7px] top-3 bottom-3 w-0.5 bg-gradient-to-b from-emerald-500 via-emerald-500/30 to-teal-500 rounded-full" />
                          ...
                          <div className="flex items-start gap-4 pl-5">
                            <div className="mt-1 h-3 w-3 rounded-full bg-teal-500 ring-4 ring-teal-500/20 shrink-0 -ml-[7px]" />
                            <div>
                      ...
                      <div className="p-6 pt-4 border-t border-border/50 bg-secondary/30">
                        <div className="flex items-center justify-between gap-4">
                          <div>
                            <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider mb-0.5">Price</p>
                          ...
                          <div className="flex items-center gap-2">
                            <Button 
                              variant="outline" 
                              size="icon" 
                              className="h-11 w-11 rounded-xl border-border/50 hover:bg-emerald-500/10 hover:text-emerald-600 hover:border-emerald-500/30 transition-all"
                            ...
                            <Button 
                              ...
                              className="h-11 rounded-xl px-5 font-bold text-xs bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-white shadow-lg shadow-emerald-500/20 border-0 transition-all hover:scale-[1.02] active:scale-[0.98]"
                            >
                              Message
